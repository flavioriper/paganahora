                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="card-box table-responsive">
                            <h4 class="m-t-0 header-title"><b>Solicitações de Pedido de Saque</b></h4>
                            <p class="text-muted font-13 m-b-30">
                                Todas pedidos de saque pendentes são mostradas aqui
                            </p>
...
<th>Ação</th>
                                ...
                                @foreach($opens as $live)
                                    <tr>
                                        ...
                                        <td>@if($live->system == 4) QIWI @elseif($live->system == 1) Chave PIX @elseif($live->system == 5) Depósito Bancário @endif</td>
										<td>{{$live->wallet}}</td>
                                        <td>{{$live->amount}}</td>
                                        <td>
                                            <a href="/admin/acceptWithdraw/{{$live->id}}" class="table-action-btn">Concluído</a>
                                            <span> | </span>
                                            <a href="/admin/declineWithdraw/{{$live->id}}" class="table-action-btn">Recusado</a>
                                        </td>
                                    </tr>
                                @endforeach
